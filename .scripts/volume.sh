...
send_notification() {
	if [ "$1" = "mute" ]; then ICON="muted-panel"; elif [ "$VOLUME" -lt 33 ]; then ICON="low-panel"; elif [ "$VOLUME" -lt 66 ]; then ICON="medium-panel"; else ICON="high-panel"; fi
	if [ "$1" = "mute" ]; then TEXT="Currently muted"; else TEXT="Currently at ${VOLUME}%"; fi

	dunstify -a "Volume" -r 9993 -h int:value:"$VOLUME" -i "volume-level-$ICON" "Volume" "$TEXT" -t 2000
}
